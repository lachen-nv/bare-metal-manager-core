...
fn handle_list(_detailed: bool, format: OutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let devices = crate::discovery::discover_devices()?;
...
pub fn resolve_device(
    device_spec: &str,
) -> Result<crate::info::MlxDeviceInfo, Box<dyn std::error::Error>> {
    // Use existing device discovery logic.
    let devices = crate::discovery::discover_devices()?;

    // Find device by PCI address.
    if let Some(found_device) = devices.iter().find(|d| {
        crate::discovery::convert_pci_name_to_address(&d.pci_name).unwrap_or_default()
            == device_spec
    }) {

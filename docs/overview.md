...
Carbide is an API-based microservice that provides site-local, zero-trust bare-metal lifecycle management with DPU-enforced isolation, allowing for deployment of multi-tenant AI infrastructure at scale. Carbide enables zero-touch automation and ensures the integrity and separation of workloads at the bare-metal layer.

## Carbide Operational Principles

Carbide has been designed according to the following principles:
...
* The network fabric (i.e. Leaf Switches and routers) stays static even during tenancy changes within Carbide.

## Carbide Responsibilities

Carbide is responsible for the following tasks in the data-center environment:
...
Carbide is not responsible for the following tasks:
...
Carbide is a service with multiple components that drive actions based on API calls, which can originate from users or
as events triggered by machines (e.g. a DHCP boot or PXE request).

Each service communicates with the Carbide API server over [gRPC](https://grpc.io) using
...
The Carbide deployment includes a number of services:

- **Carbide API service**: Allows users to
...
In addition to the Carbide service components, there are other supporting services that must be set up within the K8s site
controller nodes.
...
- The admin CLI provides a command line interface into Carbide.
...
- [Postgres](https://www.postgresql.org/): This database is used to store state for any Carbide or site controller
...
During initial deployment, the `scout` service runs, informing the Carbide API of any discovered DPUs. Carbide completes the installation of services on the DPU and boots into regular operation mode. Thereafter, the `carbide-dpu-agent` starts as a daemon.

Each DPU runs the `carbide-dpu-agent` which connects via gRPC to the API service in Carbide to get configuration
instructions.

...
Carbide is a Bare-Metal-As-A-Service (BMaaS) solution. It manages the lifecycle of hosts, including user OS installation, host cleanup, validation tests, and automated software updates. It also provides host monitoring and virtualized private networking capabilities on ethernet and InfiniBand.

In order to enable virtual private networks (overlay networks), Carbide utilizes DPUs as primary ethernet interfaces of hosts.
...
DPUs are configured by the Carbide site controller via a **declarative** and **stateless** mechanism:

- The agent running on DPUs (`carbide-dpu-agent`) requests the current desired configuration via the `GetManagedHostNetworkConfig` gRPC API call. Example data of the returned configuration is provided in the [Appendix](#dpu-configuration-example) below.
...
- The `carbide-dpu-agent` also reconfigures a DHCP server running on the DPU, which responds to DHCP requests from the attached host.
- After HBN and the DHCP server are reconfigured, `carbide-dpu-agent` implements health-checks that supervise whether the desired configurations are in-place and check whether the DPU is healthy (e.g. the agent continuously checks whether the DPU has established BGP peering with TORs and route servers according to the desired configuration).
- The `carbide-dpu-agent` uses the `RecordDpuNetworkStatus` gRPC API call to report back to the site control plane whether the desired configurations are applied, and whether all health checks are succeeding.
...
sequenceDiagram
    ...
    participant Agent as carbide-dpu-Agent
    participant Nvue as nvue
    participant Dhcp as DHCP Server

    loop Every 30s
        Agent->>Carbide: GetManagedHostNetworkConfig()\nReturns desired configs and versions
        Agent->>Nvue: Apply requested configuration
        Agent->>Dhcp: Reconfigure DHCP Server
        Agent->>Carbide: RecordDpuNetworkStatus()\nReport applied config versions\nReport DPU health
    end
```
...
- Carbide will never show a configuration as applied without feedback from the DPU. Doing so would cause reliability issues (e.g. double-assignment of IPs), as well as raise security concerns.
...
sequenceDiagram
    ...
    opt On Instance creation
        Tenant ->> Carbide: Create Instance
        Carbide ->> Carbide: Set Instance config and version\nUpdate ManagedHost config (use_admin_network: false) and increment version
        Agent->>Carbide: GetManagedHostNetworkConfig()\nReturns desired configs and versions
        Agent->>Nvue: Apply requested configuration
        Agent->>Dhcp: Reconfigure DHCP Server
        Agent->>Carbide: RecordDpuNetworkStatus()\nReport applied config versions\nReport DPU health
        Note right of Carbide: Transition Host between required states
        Carbide ->> Tenant: Report the Instance is Ready for usage
    end

    opt On Instance deletion
        Carbide ->> Carbide: Update ManagedHost config (use_admin_network: true)\nand increment version
        Agent->>Carbide: GetManagedHostNetworkConfig()\nReturns desired configs and versions
        Agent->>Nvue: Apply requested configuration
        Agent->>Dhcp: Reconfigure DHCP Server
        Agent->>Carbide: RecordDpuNetworkStatus()\nReport applied config versions\nReport DPU health
        Carbide->>Carbide: Observe that expected ManagedHost network config is applied\nTransition Host to cleanup states
...
:::{note}
This is not the only mechanism that Carbide utilizes to provide security on the networking layer. In addition to this, ACLs and routing table separation are used to implement secure virtual private networks (VPCs).
:::
